...
- [ ] T001 Create planned source folders `Prompts/`, `Context/`, `Sessions/`, `Interaction/`, `Persistence/`, `Diagnostics/`, `docs/`, `tests/unit/`, `tests/integration/`, and `tests/manual/`
- [ ] T002 Update project metadata and package settings in `ComPewter.csproj`
- [ ] T003 Update SMAPI metadata to match ComPewter v1 scope in `manifest.json`
- [ ] T004 Replace the scaffold config example with v1 defaults in `config.example.json`
- [ ] T005 [P] Create provider setup documentation stub in `docs/provider-setup.md`
- [ ] T006 [P] Create privacy documentation stub in `docs/privacy.md`
- [ ] T007 [P] Create troubleshooting documentation stub in `docs/troubleshooting.md`
- [ ] T008 [P] Create uninstall documentation stub in `docs/uninstall.md`
- [ ] T009 [P] Create developer architecture documentation stub in `docs/architecture.md`
- [ ] T010 [P] Create manual acceptance checklist from quickstart in `tests/manual/compewter-acceptance.md`
...
- [ ] T011 Replace provider enum with Disabled/Anthropic/OpenAI/Ollama/Custom values in `Config/AiProviderType.cs`
- [ ] T012 Implement provider-specific config records in `Config/ProviderSettings.cs`
- [ ] T013 Implement privacy config records in `Config/PrivacySettings.cs`
- [ ] T014 Implement UI config records in `Config/UiSettings.cs`
- [ ] T015 Expand strongly typed mod config with safe defaults in `Config/ModConfig.cs`
- [ ] T016 Implement config validation, clamping, schema versioning, and migration/default repair in `Config/ConfigValidator.cs`
- [ ] T017 [P] Implement secret redaction helpers in `Diagnostics/SecretRedactor.cs`
- [ ] T018 [P] Implement secret-safe logging wrapper in `Diagnostics/ModLogger.cs`
- [ ] T019 [P] Implement provider error to player-message mapper in `Diagnostics/ErrorMessageMapper.cs`
- [ ] T020 Define provider-neutral request/result/error contracts in `Providers/AiChatRequest.cs`, `Providers/AiChatResult.cs`, and `Providers/AiProviderError.cs`
- [ ] T021 Replace the scaffold provider interface with v1 contract in `Providers/IAiChatProvider.cs`
- [ ] T022 Implement Disabled provider setup response in `Providers/DisabledProvider.cs`
- [ ] T023 Implement provider factory selection/defaulting in `Providers/AiProviderFactory.cs`
- [ ] T024 Define session message models in `Sessions/ChatMessage.cs` and `Sessions/ChatMessageStatus.cs`
- [ ] T025 Define chat session state model in `Sessions/ChatSession.cs`
- [ ] T026 Define no-op v1 save data service in `Persistence/SaveDataService.cs`
- [ ] T027 Refactor `ModEntry.cs` into a thin composition root using config validation, provider factory, diagnostics, and hotkey interaction services
...
- [ ] T033 [P] [US1] Add manual UI hotkey/open/loading checklist in `tests/manual/us1-chat-ui.md`

### Implementation for User Story 1

- [ ] T034 [US1] Implement current-session manager submit flow, in-flight guard, timeout cancellation, and message updates in `Sessions/ChatSessionManager.cs`
- [ ] T035 [US1] Implement hotkey open behavior in `Interaction/HotkeyComputerInteraction.cs`
- [ ] T036 [US1] Replace scaffold menu with v1 chat menu shell in `UI/ChatMenu.cs`
- [ ] T037 [US1] Implement keyboard text input helper in `UI/ChatTextInput.cs`
- [ ] T038 [US1] Implement scroll state for message list in `UI/ChatScrollState.cs`
- [ ] T039 [US1] Wire chat menu to session manager loading/success/error states in `UI/ChatMenu.cs`
- [ ] T040 [US1] Update `ModEntry.cs` to use `HotkeyComputerInteraction` and open `ChatMenu`
...
- [ ] T044 [P] [US2] Add manual provider configuration checklist in `tests/manual/us2-provider-config.md`

### Implementation for User Story 2

- [ ] T045 [US2] Implement Anthropic provider validation, request construction, and successful response parsing in `Providers/AnthropicProvider.cs`
- [ ] T046 [US2] Implement OpenAI provider validation, request construction, and successful response parsing in `Providers/OpenAiProvider.cs`
- [ ] T047 [US2] Implement Ollama provider validation, request construction, and successful response parsing in `Providers/OllamaProvider.cs`
- [ ] T048 [US2] Implement Custom OpenAI-compatible validation, request construction, and successful response parsing in `Providers/CustomProvider.cs`
- [ ] T049 [US2] Add shared JSON/HTTP helper behavior for providers in `Providers/ProviderHttpClient.cs`
- [ ] T050 [US2] Update provider factory to instantiate Anthropic/OpenAI/Ollama/Custom providers in `Providers/AiProviderFactory.cs`
- [ ] T051 [US2] Implement optional Generic Mod Config Menu adapter with non-fatal load behavior in `Config/GenericModConfigMenuIntegration.cs`
- [ ] T052 [US2] Document Anthropic/OpenAI/Ollama/Custom/Disabled setup and example snippets in `docs/provider-setup.md`
...
- [ ] T056 [P] [US3] Add 10-prompt manual context/spoiler acceptance checklist in `tests/manual/us3-context-spoilers.md`

### Implementation for User Story 3

- [ ] T057 [US3] Implement prompt options and envelope model in `Prompts/PromptOptions.cs`
- [ ] T058 [US3] Implement prompt builder with system/privacy/context/history/question sections in `Prompts/PromptBuilder.cs`
- [ ] T059 [US3] Implement context snapshot and collector contract in `Context/GameContextSnapshot.cs` and `Context/IGameContextCollector.cs`
- [ ] T060 [US3] Implement aggregate context service with on-demand collection in `Context/GameContextService.cs`
- [ ] T061 [P] [US3] Implement date/weather/luck collector in `Context/DateWeatherContextCollector.cs`
- [ ] T062 [P] [US3] Implement player money/skills/location/farm type collector in `Context/PlayerContextCollector.cs`
- [ ] T063 [P] [US3] Implement compact inventory collector in `Context/InventoryContextCollector.cs`
- [ ] T064 [P] [US3] Implement active quests collector in `Context/QuestContextCollector.cs`
- [ ] T065 [P] [US3] Implement opt-in friendship collector in `Context/FriendshipContextCollector.cs`
- [ ] T066 [P] [US3] Implement progression/bundle collector where practical in `Context/ProgressionContextCollector.cs`
- [ ] T067 [P] [US3] Implement opt-in installed mods collector in `Context/ModListContextCollector.cs`
- [ ] T068 [US3] Wire context service and prompt builder into `Sessions/ChatSessionManager.cs`
- [ ] T069 [US3] Add in-chat context enable hint when context is Off in `UI/ChatMenu.cs`
- [ ] T070 [US3] Document context categories and spoiler behavior in `docs/privacy.md`
...
- [ ] T074 [P] [US4] Add manual provider failure checklist in `tests/manual/us4-provider-failures.md`

### Implementation for User Story 4

- [ ] T075 [US4] Implement HTTP status and exception mapping for Anthropic in `Providers/AnthropicProvider.cs`
- [ ] T076 [US4] Implement HTTP status and exception mapping for OpenAI in `Providers/OpenAiProvider.cs`
- [ ] T077 [US4] Implement unavailable server/model and malformed response mapping for Ollama in `Providers/OllamaProvider.cs`
- [ ] T078 [US4] Implement unsupported settings and schema mismatch mapping for Custom in `Providers/CustomProvider.cs`
- [ ] T079 [US4] Ensure all provider request logs use redacted diagnostics in `Diagnostics/ModLogger.cs`
- [ ] T080 [US4] Render retry-safe friendly errors in `UI/ChatMenu.cs`
- [ ] T081 [US4] Document provider error troubleshooting in `docs/troubleshooting.md`
...
- [ ] T084 [P] [US5] Add current-session history cap/restart checklist in `tests/manual/us5-save-multiplayer.md`
- [ ] T085 [P] [US5] Add multiplayer smoke checklist in `tests/manual/us5-save-multiplayer.md`

### Implementation for User Story 5

- [ ] T086 [US5] Ensure `Persistence/SaveDataService.cs` does not write chat history or secrets to save data in v1
- [ ] T087 [US5] Enforce current-session message cap in `Sessions/ChatSessionManager.cs`
- [ ] T088 [US5] Ensure hotkey/chat behavior is local-only and performs no world mutation in `Interaction/HotkeyComputerInteraction.cs`
- [ ] T089 [US5] Guard context collectors from sending multiplayer player data unless explicitly enabled in `Context/GameContextService.cs`
- [ ] T090 [US5] Document multiplayer limitations in `docs/privacy.md`
- [ ] T091 [US5] Document uninstall behavior and save-safety notes in `docs/uninstall.md`
...
- [ ] T093 [P] Update README with v1 hotkey-only scope, provider setup links, privacy summary, and known limitations in `README.md`
- [ ] T094 [P] Complete developer architecture documentation in `docs/architecture.md`
- [ ] T095 [P] Add release readiness checklist in `tests/manual/release-readiness.md`
- [ ] T096 Run `dotnet build` and record result in `tests/manual/release-readiness.md`
- [ ] T097 Run full manual acceptance checklist from `quickstart.md` and record results in `tests/manual/release-readiness.md`
- [ ] T098 Review all docs for secret-handling/privacy consistency in `docs/provider-setup.md`, `docs/privacy.md`, and `docs/troubleshooting.md`
- [ ] T099 Review generated mod package contents for config/example/docs correctness in `tests/manual/release-readiness.md`
